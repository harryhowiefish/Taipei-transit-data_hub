...
def E_mrt_parking_info():
    ...
    df["GetDatatime"] = now
    print("E_mrt_parking_info finished")
    return (df)


def T_mrt_parking_info(df: pd.DataFrame):
    ...
    df = df.loc[:, ["ParkName", "StationNo", "StationName", "line_type",
                    "ParkType", "ParkTotalNo", "GetDatatime",]]
    df.rename(columns={
        "ParkName": "park_name",
    ...
    # return ("OK")
    print("T_mrt_parking_info finished")
    return (df)  # 可以輸出df做load之用  測試階段先直接輸出csv到local端


def L_mrt_parking_info(df: pd.DataFrame, port: str = "docker"):
    ...
    with create_engine(f"mysql+pymysql://{username_sql}:{password_sql}@{server}/{db_name}",).connect() as conn:
        df.to_sql(
            name="mrt_parking_info",
            con=conn,
            if_exists="replace",
            index=False
        )
    print("L_mrt_realtime_parking_info finished")
