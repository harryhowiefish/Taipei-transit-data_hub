...
def update_data_insert_merge_into_ods(dataset_name: str, source_table_name: str, target_table_name: str, client: bigquery.Client):
    """USE MERGE FUNCTION to insert new data from src into youbike_ods"""
    query_job = client.query(
        f"""
    MERGE INTO `{dataset_name}.{target_table_name}` AS target
    USING `{dataset_name}.{source_table_name}` AS source
    ...
    print(
        f"new data has been insertde into {dataset_name}.{target_table_name}")
    return (result)


def update_data_insert_merge_into_fact_bike_realtime(dataset_name: str, source_table_name: str, target_table_name: str, client: bigquery.Client):
...
if __name__ == "__main__":
    ...
    update_data_insert_merge_into_ods(dataset_name="Youbike",
                                      source_table_name="SRC_youbike_after0504", target_table_name="ODS_youbike_realtime", client=BQ_CLIENT)
    update_data_insert_merge_into_fact_bike_realtime(dataset_name="Youbike",
                                                     source_table_name="ODS_youbike_realtime", target_table_name="FACT_bike_realtime", client=BQ_CLIENT)
